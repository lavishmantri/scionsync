...
var _WebSocketClient = class _WebSocketClient {
  // 25 seconds
  constructor(serverUrl, vaultName, deviceId) {
    this.socket = null;
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        const timeout = setTimeout(() => {
          var _a;
          if (((_a = this.socket) == null ? void 0 : _a.readyState) === WebSocket.CONNECTING) {
            this.socket.close();
   ...
   */
  getWebSocketUrl() {
    let wsUrl = this.serverUrl.replace(/^http:/, "ws:").replace(/^https:/, "wss:");
    wsUrl = wsUrl.replace(/\/$/, "");
   ...
   */
  scheduleReconnect() {
    if (this.reconnectAttempts >= _WebSocketClient.MAX_RECONNECT_ATTEMPTS) {
    ...
    const delay = Math.min(
      _WebSocketClient.BASE_RECONNECT_DELAY * Math.pow(2, this.reconnectAttempts),
      _WebSocketClient.MAX_RECONNECT_DELAY
    );
  ...
  startPingInterval() {
    ...
    }, _WebSocketClient.PING_INTERVAL);
  }
  /**
   * Stop ping interval
   */
...
_WebSocketClient.MAX_RECONNECT_ATTEMPTS = 10;
_WebSocketClient.BASE_RECONNECT_DELAY = 1e3;
// 1 second
_WebSocketClient.MAX_RECONNECT_DELAY = 3e4;
// 30 seconds
_WebSocketClient.PING_INTERVAL = 25e3;
var WebSocketClient = _WebSocketClient;
function generateDeviceId() {
  const array = new Uint8Array(8);
  crypto.getRandomValues(array);
  const random2 = Array.from(array).map((b) => b.toString(16).padStart(2, "0")).join("");
  return `device-${random2}`;
}
...
// src/yjs-manager.ts
var YjsManager = class {
  // fileId -> YjsDocumentInfo
  constructor() {
  ...
  getDoc(fileId) {
    ...
    return (_a = info == null ? void 0 : info.doc) != null ? _a : null;
  }
  /**
   ...
   */
  applyLocalChange(fileId, newContent) {
    const info = this.docs.get(fileId);
    if (!info) {
      const doc3 = this.getOrCreateDoc(fileId, newContent);
      info.isDirty = true;
      return encodeStateAsUpdate(doc3);
    }
    const doc2 = info.doc;
    const text2 = doc2.getText("content");
    const currentContent = text2.toString();
    if (currentContent === newContent) {
      return null;
    }
    doc2.transact(() => {
      text2.delete(0, text2.length);
      text2.insert(0, newContent);
    });
    info.isDirty = true;
    if (info.lastSyncedStateVector) {
      return encodeStateAsUpdate(doc2, info.lastSyncedStateVector);
    } else {
   ...
   */
  applyRemoteUpdate(fileId, update) {
    const doc2 = this.getOrCreateDoc(fileId);
    const info = this.docs.get(fileId);
    applyUpdate(doc2, update);
    info.lastSyncedStateVector = encodeStateVector(doc2);
    info.isDirty = false;
    return doc2.getText("content").toString();
  }
  /**
   * Initialize a document from server state
   * Called when first syncing a file
   */
  initFromServerState(fileId, serverState) {
    this.docs.delete(fileId);
    const doc2 = new Doc();
    applyUpdate(doc2, serverState);
    const info = {
      fileId,
      doc: doc2,
      lastSyncedStateVector: encodeStateVector(doc2),
      pendingUpdates: [],
      isDirty: false
    };
    this.docs.set(fileId, info);
  ...
  getStateVector(fileId) {
    const info = this.docs.get(fileId);
    if (!info) return null;
    return encodeStateVector(info.doc);
  }
  /**
  ...
  getFullState(fileId) {
    const info = this.docs.get(fileId);
    if (!info) return null;
    return encodeStateAsUpdate(info.doc);
  }
  /**
  ...
  markSynced(fileId) {
    const info = this.docs.get(fileId);
    if (!info) return;
    info.lastSyncedStateVector = encodeStateVector(info.doc);
    info.isDirty = false;
    info.pendingUpdates = [];
  }
  /**
  ...
  isDirty(fileId) {
    var _a;
    const info = this.docs.get(fileId);
    return (_a = info == null ? void 0 : info.isDirty) != null ? _a : false;
  }
  /**
...
function shouldUseYjs(filePath) {
...
// src/structure-crdt.ts
var StructureCRDT = class {
  constructor() {
    ...
    this.doc.on("update", (update, origin) => {
      var _a;
      if (origin !== "remote") {
        (_a = this.updateCallback) == null ? void 0 : _a.call(this, update);
      }
    });
   ...
   */
  initFromServerState(serverState) {
    this.doc.transact(() => {
      this.files.clear();
    });
    applyUpdate(this.doc, serverState, "remote");
   ...
   */
  addFile(fileId, filePath, entryType, hash) {
    const now = Date.now();
    const entry = {
      file_id: fileId,
      path: filePath,
  ...
  getFileIdByPath(filePath) {
    ...
    return entry == null ? void 0 : entry.file_id;
  }
  /**
   ...
   */
  getFilesToRename(localPathsToFileId) {
    const toRename = [];
...
function structureUint8ArrayToBase64(arr) {
  let binary = "";
  for (let i = 0; i < arr.length; i++) {
    binary += String.fromCharCode(arr[i]);
  }
...
function structureBase64ToUint8Array(base64) {
  const binary = atob(base64);
  const arr = new Uint8Array(binary.length);
  for (let i = 0; i < binary.length; i++) {
    arr[i] = binary.charCodeAt(i);
...
var OfflineQueue = class {
  ...
  remove(id2) {
    ...
    this.queue = this.queue.filter((op) => op.id !== id2);
    if (this.queue.length < before) {
      console.log(`[OfflineQueue] Removed operation ${id2}`);
      this.persist();
    }
  }
  /**
   ...
   */
  getStatus() {
    const oldestTimestamp = this.queue.length > 0 ? Math.min(...this.queue.map((op) => op.timestamp)) : null;
...
var _SyncService = class _SyncService {
  constructor(app, settings, vaultName, syncState, saveDataFn, getDeviceIdFn, setDeviceIdFn, getQueueFn, setQueueFn) {
    this.syncState = {};
  ...
  async initializeWebSocket() {
    ...
    this.wsClient.onConnectionStateChange(async (state) => {
      ...
      (_a = this.connectionStateCallback) == null ? void 0 : _a.call(this, state);
      if (state === "connected") {
        ...
        if (queueStatus.count > 0) {
          ...
          if (result.processed > 0) {
            new import_obsidian.Notice(`Scion Sync: Synced ${result.processed} offline changes`);
          }
          if (result.failed > 0) {
            new import_obsidian.Notice(`Scion Sync: ${result.failed} offline changes failed to sync`);
          }
        }
   ...
   */
  async handleStructureUpdate(message) {
    if (!message.payload) {
   ...
   */
  async syncFilesystemToStructure() {
    ...
    const localPathToFileId = /* @__PURE__ */ new Map();
    for (const [path, state] of Object.entries(this.syncState)) {
   ...
   */
  async handleBinaryUpdate(message) {
    if (!message.fileId) {
    ...
    if (message.payload && message.payload.startsWith("conflict:")) {
      const conflictPath = message.payload.substring(9);
      new import_obsidian.Notice(`Binary conflict: ${message.fileId}
Conflict copy saved as: ${conflictPath}`);
   ...
   */
  async sendYjsUpdate(path) {
    const localState = this.syncState[path];
    const fileId = localState == null ? void 0 : localState.file_id;
    if (!fileId) {
   ...
   */
  queueOfflineOperation(type, fileId, path, payload) {
  ...
  isWebSocketConnected() {
    var _a, _b;
    return (_b = (_a = this.wsClient) == null ? void 0 : _a.isConnected()) != null ? _b : false;
  }
  /**
  ...
  getWebSocketState() {
    var _a, _b;
    return (_b = (_a = this.wsClient) == null ? void 0 : _a.getConnectionState()) != null ? _b : "disconnected";
  }
  /**
   * Get device ID
   */
...
var DEFAULT_SETTINGS = {
  ...
  debounceInterval: 3,
  useWebSocket: true
  // Enable real-time sync by default
};
var ScionSyncPlugin = class extends import_obsidian2.Plugin {
  ...
  async loadSettings() {
    ...
    this.deviceId = (data == null ? void 0 : data.deviceId) || "";
    this.offlineQueue = (data == null ? void 0 : data.offlineQueue) || [];
  ...
  getStatusText(status) {
    if (this.settings.useWebSocket) {
      const wsIcon = this.wsConnectionState === "connected" ? "\u25CF" : this.wsConnectionState === "connecting" || this.wsConnectionState === "reconnecting" ? "\u25CB" : "\u25CB";
...
var ScionSyncSettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian2.Setting(containerEl).setName("Server URL").setDesc("The URL of your Scion sync server (e.g., http://192.168.1.100:3000)").addText(
      (text2) => text2.setPlaceholder("http://localhost:3000").setValue(this.plugin.settings.serverUrl).onChange(async (value) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Real-time sync (WebSocket)").setDesc("Enable instant sync via WebSocket connection. Falls back to polling if unavailable.").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.useWebSocket).onChange(async (value) => {
        var _a2;
...
var SyncStatusModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    infoEl.createEl("p", { text: `Device ID: ${((_b = this.plugin.getSyncService()) == null ? void 0 : _b.getDeviceId().substring(0, 16)) || "N/A"}...` });
    ...
    if (this.plugin.settings.useWebSocket) {
      const wsState = ((_c = this.plugin.getSyncService()) == null ? void 0 : _c.getWebSocketState()) || "disconnected";
      const wsStatus = wsState === "connected" ? "Connected" : wsState === "connecting" ? "Connecting..." : wsState === "reconnecting" ? "Reconnecting..." : "Disconnected";
    ...
    if (stats) {
      ...
      statsEl.createEl("p", { text: `Last server commit: ${((_d = stats.lastCommit) == null ? void 0 : _d.substring(0, 8)) || "None"}` });
